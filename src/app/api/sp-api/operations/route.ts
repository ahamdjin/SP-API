  ...
  try {
    ...
    switch (input.operation) {
      case "inventory": {
        ...
        addOptional(params, "nextToken", optionalString(fields, "nextToken"));
      ...
      case "orders": {
        ...
        addOptional(params, "paginationToken", optionalString(fields, "paginationToken"));
        result = await call(input, "/orders/2026-01-01/orders?" + params);
        break;
      }
      case "order": {
        const orderId = stringField(fields, "orderId");
        result = await call(input, "/orders/2026-01-01/orders/" + encodeURIComponent(orderId));
        break;
      }
      case "reports": {
        const params = new URLSearchParams({
          reportTypes: stringField(fields, "reportTypes"),
          marketplaceIds: input.marketplaceId,
      ...
      case "feeds": {
        const params = new URLSearchParams({
          feedTypes: stringField(fields, "feedTypes"),
          marketplaceIds: input.marketplaceId,
          pageSize: String(numberField(fields, "pageSize", 1, 100, 20)),
        });
        addCsv(params, "processingStatuses", optionalString(fields, "processingStatuses"), 5);
      ...
      case "itemLabels": {
        ...
        result = await call(input, "/inbound/fba/2024-03-20/items/labels", "POST", {
          ...
          mskuQuantities: parseItems(stringField(fields, "items")).map(({ msku, quantity }) => ({ msku, quantity })),
        });
        break;
      }
      case "shipmentLabels": {
        ...
        addOptional(params, "NumberOfPackages", optionalString(fields, "numberOfPackages"));
        addOptional(params, "NumberOfPallets", optionalString(fields, "numberOfPallets"));
...
async function submitFeed(input: Input, fields: Fields) {
  ...
  const upload = await fetch(url, {
    ...
    body: stringField(fields, "content"),
    cache: "no-store",
  });
...
function addCsv(params: URLSearchParams, key: string, value: string, max: number, repeated = false) {
  if (!value) return;
  const values = value.split(/[\n,]/).map((entry) => entry.trim()).filter(Boolean).slice(0, max);
...
function parseItems(value: string) {
  const items = value.split(/\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    ...
    if (!msku || !Number.isInteger(quantity) || quantity < 1) throw new SpApiError("Each item must use: MSKU, quantity, prep owner, label owner", 400);
